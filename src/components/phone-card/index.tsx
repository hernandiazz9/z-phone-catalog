...
export function PhoneCard({ phone, priority = false }: PhoneCardProps) {
  ...
  return (
    <article className="bg-background relative">
      <Link
        href={`/phone/${phone.id}`}
        className="group focus-visible:outline-foreground relative flex aspect-square flex-col gap-6 overflow-hidden p-4 focus-visible:outline-2 focus-visible:outline-offset-2"
      >
        <span
          aria-hidden="true"
        ...
        <div className="relative flex items-end gap-2 text-xs tracking-widest uppercase">
          <div className="flex min-w-0 flex-1 flex-col gap-1">
            <span
              className={`text-muted text-2xs tracking-brand truncate ${BRAND_TO_MUTED_INVERSE}`}
            >
              {phone.brand}
            </span>
      ...
      <div className="absolute top-3 right-3 z-10">
        <QuickAddButton phone={{ id: phone.id, brand: phone.brand, name: phone.name }} />
      </div>
    </article>
  )
}
